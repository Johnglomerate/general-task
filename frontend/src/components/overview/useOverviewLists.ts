...
const useOverviewLists = () => {
    ...
    const isLoading = isListsLoading || isSettingsLoading || isActiveTasksLoading

    const sortedAndFilteredLists = useMemo(() => {
        if (isListsLoading || isSettingsLoading || isActiveTasksLoading || !lists || !settings) return []
    ...
    }, [lists, isLoading, settings, overviewAutomaticEmptySort, activeTasks])
